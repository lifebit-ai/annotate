...
ch_N_samples = Channel.fromPath("${siteqc_results_dir}/*${N_samples_suffix}")
    .map { path -> [file(path).simpleName.split('_N').first(), file(path)] }
...
// Second, this tuple of four files is joined with all metrics files from SiteQC pipeline
// that were generated from same input bcf file.
